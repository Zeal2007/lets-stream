...
export const videoSources: VideoSource[] = [
  ...
  },
  {
    ...
    name: "AutoEmbed",
    getMovieUrl: (id: number) =>
      `https://player.autoembed.cc/embed/movie/${id}?autoplay=true`,
    getTVUrl: (id: number, season: number, episode: number) =>
  ...
  },
  {
    ...
    name: "VidSrc.xyz",
    getMovieUrl: (id: number) =>
      `https://vidsrc.xyz/embed/movie?tmdb=${id}&ds_lang=en`,
    getTVUrl: (id: number, season: number, episode: number) =>
  ...
  },
  {
    key: "letsembed",
    name: "Letsembed",
    getMovieUrl: (id: number) => `https://letsembed.cc/embed/movie?id=${id}`,
    getTVUrl: (id: number, season: number, episode: number) =>
      `https://letsembed.cc/embed/tv?id=${id}/${season}/${episode}`,
  },
  {
    key: "vidjoy",
    name: "VidJoy",
...
export const getPopularMovies = async (page: number = 1): Promise<Media[]> => {
  try {
    ...
    return data.results.map((item: TMDBMovieResult) =>
      formatMediaItem({ ...item, media_type: "movie" })
    );
  } catch (error) {
...
export const getPopularTVShows = async (page: number = 1): Promise<Media[]> => {
  try {
    ...
    const data = await response.json();
    return data.results.map((item: TMDBTVResult) =>
      formatMediaItem({ ...item, media_type: "tv" })
    );
  } catch (error) {
...
export const getMovieDetails = async (
  ...
  try {
    ...
    if (!detailsResponse.ok || !imagesResponse.ok) {
      console.error(
        `API error: Details ${detailsResponse.status}, Images ${imagesResponse.status}`
      );
      return null;
    }
    ...
    const formattedData = formatMediaItem({
      ...detailsData,
      media_type: "movie",
    });
    ...
    return {
      ...
      release_date:
        formattedData.release_date || detailsData.release_date || "", // Ensure release_date is never undefined
...
export const getTVDetails = async (id: number): Promise<TVDetails | null> => {
  try {
    ...
    return {
      ...
      first_air_date:
        formattedData.first_air_date || detailsData.first_air_date || "", // Ensure first_air_date is never undefined
...
export const validateTMDBId = async (
  mediaType: "movie" | "tv",
  tmdbId: number
) => {
  try {
